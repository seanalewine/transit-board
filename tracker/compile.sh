...
if [ -s "$TEMP_OUTPUT_FILE" ]; then
    ...
    # --- STEP 2: Map nextStaId and line to unifiedId ---
    echo "Mapping 'nextStaId' and 'line' to 'unifiedId' using map file $MAP_FILE"
    ...
    jq --slurpfile id_map "$MAP_FILE" '
        ...
        ($id_map[0]) as $map |
        # NEW: Construct the lookup key by concatenating nextStaId and the original line code
        # The original line code is the part of .output_color before the config color is applied (e.g., "pink" from "#E27BCE")
        # We assume the config color is applied *after* this step in the original script
        # NOTE: Since STEP 1 already applied the hex color, we must use the 'output_color' from the previous un-colored file for the line code.
        # Rerunning the train loop to get the original line code into a new field is cleaner, 
        # but for simplicity, we'll try to **revert the lookup logic** by assuming the original LINE_CODE is available.
        # Since the original LINE_CODE is *not* present here, we must rely on the fact that the line code is still in the input JSON from the loop above.
        
        # ***CRITICAL FIX: Rework the data flow to preserve the raw line code for mapping.***
        # The line code is required *before* it's converted to a hex color in STEP 1.
        # I will perform the ID mapping **before** the color application.
        # Let's move the ID mapping (STEP 2) before the color application (STEP 1).
        
        # ***REVISING THE PLAN:***
        # 1. Update the 'awk' script (DONE)
        # 2. Modify the main loop's 'jq' to output a new field `line_code` with the raw line abbr. (e.g., "pink"). (Not needed, `.output_color` already has it before step 1).
        # 3. **Swap the order of STEP 1 and STEP 2.**

        # Since the script is long, I will now re-order the steps and correct the data flow.
        
        # ***WAIT***: I cannot easily swap the steps in the current script structure without rewriting the entire logic.
        # I will instead create a **new field** in the first loop with the raw line code to fix the data dependency.
        
        # New Plan:
        # 1. **Awk script:** Key is `nextStaId:line`, Value is `unifiedId` (DONE).
        # 2. **Initial Loop JQ:** Add a new field, `line_code`, which holds the raw line abbreviation (e.g., "red", "g").
        # 3. **STEP 1 (Map nextStaId):** This is the **new Step 1**. It uses `nextStaId` and the new `line_code` to create the lookup key.
        # 4. **STEP 2 (Apply Colors):** This is the **new Step 2**. It uses `line_code` to set `output_color`.
        
        # Re-integrating the corrected steps into the full script:

# --- Script Update for Initial Loop ---
# Inside the "Loop through each train line abbreviation" section:
# OLD JQ:
# jq '
#     .ctatt.route[] |
#     select(.train) |
...
# --- Script Update for Step 2 (Now Step 1) ---
# OLD STEP 2 (Mapping) now becomes the first manipulation after the loop.
# It will map the ID, then output to a new file, say TEMP_MAPPED_FILE.
...
if [ -s "$TEMP_OUTPUT_FILE" ]; then
    ...
    jq --slurpfile id_map "$MAP_FILE" '
        ...
        | del(.line_code)
    ' "$TEMP_OUTPUT_FILE" > "$TEMP_MAPPED_FILE"
