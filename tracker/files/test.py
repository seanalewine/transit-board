...
# --- NEW FEATURE: Color Conversion Function ---
def convert_256_to_percent(rgb_string):
    """
    Converts a comma-separated RGB string (0-255 values) to a 
    comma-separated percentage string (0-100%).
    
    Example: "255, 128, 0" -> "100.0%, 50.2%, 0.0%"

    Args:
        rgb_string (str): A string of color values like "R, G, B".

    Returns:
        str: A string of percentage values like "R%, G%, B%".
    """
    try:
        # Split the string, strip whitespace, and convert to integers
        channels = [int(c.strip()) for c in rgb_string.split(',')]
        
        # Ensure there are 3 channels and they are within the 0-255 range
        if len(channels) != 3 or not all(0 <= c <= 255 for c in channels):
            return "0%, 0%, 0%" # Return a default if input is invalid

        # The conversion formula: (value / 255) * 100
        # Use 255.0 for float division
        percent_values = [
            round((c / 255.0) * 100, 1) for c in channels
        ]

        # Format the output as a comma-separated string with "%" sign
        return ", ".join(f"{p}%" for p in percent_values)

    except Exception as e:
        print(f"Warning: Failed to convert color string '{rgb_string}'. Error: {e}")
        return "0%, 0%, 0%"
...
def process_train_data(input_dir, station_map):
    ...
    for line_code in train_lines:
        ...
        for route in routes:
            ...
            # Get the color string from the configuration
            rgb_255_string = COLORS.get(line_code, line_code) 
            
            # --- NEW FEATURE INTEGRATION ---
            percent_string = convert_256_to_percent(rgb_255_string)
            ...
            for train in trains:
                ...
                all_trains.append({
                    ...
                    "output_color_rgb": rgb_255_string, # Original RGB string (0-255)
                    "output_color_percent": percent_string, # NEW: Percentage string (0-100%)
                    "value": value
                })
